...
def _get_tweet_user_obj(tw_obj: dict, res: dict) -> dict:
    ...
    for path in (
        ...
        if not isinstance(user_obj, dict):
            continue
        if user_obj.get("__typename") == "UserUnavailable":
            continue
...
def _parse_items(rep: httpx.Response | dict, kind: str, limit: int = -1):
    ...
    timeline_tweet_ids: set[str] = obj.get("timeline_tweet_ids", set())
    ...
    for x in obj[key].values():
        if (
            kind == "tweet"
            and x.get("id_str") in retweeted_ids
            and x.get("id_str") not in timeline_tweet_ids
        ):
            continue
